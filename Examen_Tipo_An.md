...
SELECT 
    e.puesto,
    COUNT(DISTINCT m.id) AS municipios_diferentes
FROM empleados e
JOIN sucursal s ON e.sucursalid = s.id
JOIN municipio m ON s.municipioid = m.id
GROUP BY e.puesto
ORDER BY municipios_diferentes DESC;
...
```sql
SELECT c.nombre AS cliente, c.email, c.telefono, m.nombre AS municipio
FROM clientes c
...
WHERE m.id IN (
    ...
    FROM empleados e
    JOIN sucursal s ON e.sucursalid = s.id
    GROUP BY s.municipioid
    HAVING AVG(e.salario) > 1900000
);
...
FROM empleados
GROUP BY puesto
HAVING MAX(salario) > (SELECT AVG(salario) FROM empleados);
...
SELECT DISTINCT e.empleado_id, e.nombre, e.puesto, e.fecha_contratacion, e.salario, e.sucursalid
FROM empleados e
JOIN sucursal s ON e.sucursalid = s.id
JOIN clientes c ON s.municipioid = c.municipioid
GROUP BY e.empleado_id, e.nombre, e.puesto, e.fecha_contratacion, e.salario, e.sucursalid
HAVING COUNT(DISTINCT c.cliente_id) > 9;
